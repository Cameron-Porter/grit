...
export default async function ProgramDetail({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  const[{data:days,error:dayError},{data:catalog,error:catalogError}]=await Promise.all([
  ...
  return <main className="content"><a className="back-link" href="/programs">← Programs</a><header className="page-header"><div><div className="eyebrow">{program.focus}</div><h1>{program.name}</h1><p>{program.total_weeks} weeks · {program.days_per_week} days/week</p></div>{isComplete?<span className="pill">Complete</span>:program.is_current&&<span className="pill">Active</span>}</header>
    {query.error&&<p className="notice error" role="alert">{String(query.error)}</p>}
    <section className="surface program-settings"><form className="rename-program" action={renameProgram}><input type="hidden" name="id" value={id}/><label>Program name<input name="name" defaultValue={program.name} required/></label><button className="secondary">Rename</button></form><div className="program-action-row">{!program.is_current&&!isComplete&&<form action={setCurrentProgram}><input type="hidden" name="id" value={id}/><button>Make active</button></form>}<form action={softDeleteProgram}><input type="hidden" name="id" value={id}/><button className="danger">Delete program</button></form></div></section>
    <section className="surface schedule-board" aria-labelledby="schedule-title"><header><div><div className="eyebrow">SCHEDULE</div><h2 id="schedule-title">Program overview</h2></div><div className="schedule-legend"><span className="completed">Completed</span><span className="skipped">Skipped</span><span className="next">Up next</span></div></header><div className="schedule-scroll"><table><thead><tr><th>Week</th>{Array.from({length:program.days_per_week},(_,index)=><th key={index}>Day {index+1}</th>)}</tr></thead><tbody>{weeks.map(week=><tr key={week.number}><th><strong>{week.number}</strong><span>{week.days.filter(day=>day.completed).length}/{week.days.length}</span></th>{week.days.map(day=>{const status=day.completed?'completed':day.skipped?'skipped':day.id===nextDay?.id?'next':'not-started';return <td key={day.id}><div className={`schedule-day ${status}`}><strong>{day.label??`Day ${day.day_number}`}</strong><span>{status==='completed'?'Completed':status==='skipped'?'Skipped':status==='next'?'Up next':'Not started'}</span></div></td>})}</tr>)}</tbody></table></div></section>
    <details className="surface template-editor"><summary>Edit exercise template</summary><p>Swap within the same muscle group to preserve the program’s volume. Changes apply to matching future days.</p><div className="stack day-stack">{weekOne.map(day=><section className="template-day" key={day.id}><h2>{day.label??`Day ${day.day_number}`}</h2><div className="review-exercises">{[...(day.program_exercises??[])].sort((a,b)=>(a.sort_order??0)-(b.sort_order??0)).map(exercise=><TemplateExerciseEditor key={exercise.id} programId={id} exercise={exercise} catalog={catalog??[]}/>)}</div><DayExerciseStager programId={id} dayId={day.id} catalog={catalog??[]}/></section>)}</div></details>
  </main>;
}
