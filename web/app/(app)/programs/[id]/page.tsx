...
export default async function ProgramDetail({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  const[{data:days,error:dayError},{data:catalog,error:catalogError},{data:profile,error:profileError},{data:feedbackRows,error:feedbackError}]=await Promise.all([
  ...
  const muscleFeedback=feedbackRows??[];
