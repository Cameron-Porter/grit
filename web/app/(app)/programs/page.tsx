...
export default async function Programs({ searchParams }:{ searchParams:Promise<Record<string,string|string[]|undefined>> }) {
  ...
  const { data,error } = await supabase.from('programs').select('id,name,total_weeks,days_per_week,is_current,focus,program_days(completed,skipped)').eq('user_id',user.id).is('deleted_at',null).order('created_at',{ ascending:false });
  if(error)throw new Error('Could not load programs.');
  return <main className="content">
    <header className="page-header"><div><div className="eyebrow">TRAINING</div><h1>Programs</h1></div><Link className="primary compact" href="/programs/ai">Build with AI</Link></header>
