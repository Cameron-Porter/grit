'use server';
import{randomUUID}from'node:crypto';import{revalidatePath}from'next/cache';import{redirect}from'next/navigation';import{requireUser}from'@/lib/auth/require-user';import{buildProgramExerciseRows,parseStagedExerciseItems}from'@/lib/programs/day-template-payload';
...
export async function addProgramExercises(formData:FormData){
  ...
  const{data:catalog,error:catalogError}=await supabase.from('exercises').select('id,name,muscle_group,equipment').in('id',exerciseIds);
  if(catalogError)detailFail('Could not verify the exercises.');
