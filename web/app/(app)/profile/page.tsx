...
export default async function Profile({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="content native-profile-page native-gradient-background"><header className="page-header native-profile-hero">{user.avatarUrl ? <img className="native-avatar" src={user.avatarUrl} alt="" aria-hidden="true" width={64} height={64} referrerPolicy="no-referrer"/> : <div className="native-avatar" aria-hidden="true">{(user.name ?? profile?.email ?? user.email ?? '?').trim().charAt(0).toUpperCase()}</div>}<div><div className="eyebrow">ACCOUNT</div><h1>{user.name ?? 'Profile'}</h1><p>{profile?.email ?? user.email}</p></div></header>
    ...
    <section className="surface settings-list native-settings-group"><Link className="native-settings-row native-settings-link" href="/progress"><span>Personal records</span><strong aria-hidden="true">›</strong></Link><div className="native-settings-row"><span>Membership</span><strong>{active ? 'Pro' : 'Free'}</strong></div><div className="native-settings-row"><span>Offline workout recovery</span><strong>Enabled</strong></div></section>
