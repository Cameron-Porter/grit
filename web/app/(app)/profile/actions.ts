...
export async function saveProfile(formData:FormData){
  ...
  const bodyWeightRaw=String(formData.get('bodyWeight')??'').trim(),experience=String(formData.get('experience')??''),bodyWeight=bodyWeightRaw===''?null:Number(bodyWeightRaw),autoMatchWeight=formData.get('autoMatchWeight')==='on';
  ...
  const{error}=await supabase.from('user_profiles').update({body_weight:bodyWeight,experience_level:experience,auto_match_weight:autoMatchWeight,use_preferred_equipment:settings.enabled,preferred_equipment:settings.preferred}).eq('id',user.id);
