...
export function CustomSelect({ options, value, defaultValue, onChange, name, ariaLabel, autoFocus = false }: { options: SelectOption[]; value?: string; defaultValue?: string; onChange?: (value: string) => void; name?: string; ariaLabel?: string; autoFocus?: boolean }) {
  ...
  return <div className={`custom-select ${open ? 'open' : ''}`} ref={root}>
    ...
    <button type="button" className="custom-select-trigger" aria-label={ariaLabel} role="combobox" aria-haspopup="listbox" aria-expanded={open} aria-controls={listId} aria-activedescendant={open ? optionId(active) : undefined} autoFocus={autoFocus} onClick={() => { setActive(selectedIndex); setOpen(current => !current); }} onKeyDown={event => { if (event.key === 'ArrowDown' || event.key === 'ArrowUp') { event.preventDefault(); if (!open) setOpen(true); else move(event.key === 'ArrowDown' ? 1 : -1); } if (event.key === 'Enter' && open) { event.preventDefault(); const option = options[active]; if (option) choose(option); } if (event.key === 'Escape') setOpen(false); }}>
      <span>{options[selectedIndex]?.label ?? 'Choose…'}</span><span className="custom-select-chevron" aria-hidden />
    </button>
    {open && <div className="custom-select-menu" id={listId} role="listbox" aria-label={ariaLabel}>{options.map((option, index) => <button type="button" id={optionId(index)} role="option" aria-selected={option.value === selected} disabled={option.disabled} className={`custom-select-option ${index === active ? 'active' : ''}`} key={option.value} onPointerEnter={() => setActive(index)} onClick={() => choose(option)}><span>{option.label}</span>{option.value === selected && <span aria-hidden>✓</span>}</button>)}</div>}
  </div>;
}
