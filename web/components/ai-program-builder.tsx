...
export function AiProgramBuilder({ experienceLevel, initialPriorities, catalog, history }: { experienceLevel: AiBuilderInput['experienceLevel']; initialPriorities: AiBuilderInput['priorities']; catalog: AiCatalogExercise[]; history: HistoryItem[] }) {
  ...
  return <main className="content ai-builder native-page native-gradient-background">
    ...
    <section className="surface ai-builder-form native-settings-group">
      ...
      <details className="priority-editor native-inline-menu"><summary>Muscle priorities</summary><div className="priority-grid">{MUSCLES.map(muscle => <label key={muscle}>{muscle}<CustomSelect value={input.priorities[muscle] ?? 'maintain'} onChange={value => update('priorities', { ...input.priorities, [muscle]: value as 'maintain' | 'grow' | 'emphasize' })} options={[{value:'maintain',label:'Maintain'},{value:'grow',label:'Grow'},{value:'emphasize',label:'Emphasize'}]} /></label>)}</div></details>
    ...
    {selection && preview && <section className="ai-preview native-section">
      <div className="surface native-list-card"><div className="eyebrow">VALIDATED PREVIEW</div><h2>{input.name}</h2><p>{selection.summary}</p>{preview.validation.issues.filter(issue => issue.severity === 'warning').map(issue => <p className="validation-warning" key={issue.message}>Review: {issue.message}</p>)}</div>
      {preview.days.map(day => { const selectedDay = selection.days.find(item => item.dayIndex === day.dayIndex)!; const chosen = new Map(selectedDay.selections.map(item => [item.slotId, item])); return <article className="surface native-section" key={day.dayIndex}><h3>{selectedDay.label}</h3><p>{day.splitName} · {day.totalSets} sets · about {day.estimatedMinutes} min</p><div className="ai-exercise-list">{day.slots.map(slot => { const item = chosen.get(slot.id)!; return <div className="native-settings-row" key={slot.id}><span><strong>{item.exerciseName}</strong><small>{slot.muscle} · {slot.role}</small></span><span>{slot.sets} × {slot.repsMin}–{slot.repsMax} · RIR {slot.rir}<small>{item.reason}</small></span></div>; })}</div></article>; })}
