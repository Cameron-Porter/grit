...
export function AiProgramBuilder({ experienceLevel, initialPriorities, catalog, history }: { experienceLevel: AiBuilderInput['experienceLevel']; initialPriorities: AiBuilderInput['priorities']; catalog: AiCatalogExercise[]; history: HistoryItem[] }) {
  ...
  return <main className="content ai-builder">
    <a className="back-link" href="/programs">← Programs</a>
    <header className="page-header"><div><div className="eyebrow">RULES-GROUNDED AI</div><h1>Build a program</h1><p>GRIT locks the training science. AI personalizes exercise selection.</p></div></header>
    ...
    {selection && preview && <section className="ai-preview">
      <div className="surface"><div className="eyebrow">VALIDATED PREVIEW</div><h2>{input.name}</h2><p>{selection.summary}</p>{preview.validation.issues.filter(issue => issue.severity === 'warning').map(issue => <p className="validation-warning" key={issue.message}>Review: {issue.message}</p>)}</div>
      {preview.days.map(day => { const selectedDay = selection.days.find(item => item.dayIndex === day.dayIndex)!; const chosen = new Map(selectedDay.selections.map(item => [item.slotId, item])); return <article className="surface" key={day.dayIndex}><h3>{selectedDay.label}</h3><p>{day.splitName} · {day.totalSets} sets · about {day.estimatedMinutes} min</p><div className="ai-exercise-list">{day.slots.map(slot => { const item = chosen.get(slot.id)!; return <div key={slot.id}><span><strong>{item.exerciseName}</strong><small>{slot.muscle} · {slot.role}</small></span><span>{slot.sets} × {slot.repsMin}–{slot.repsMax} · RIR {slot.rir}<small>{item.reason}</small></span></div>; })}</div></article>; })}
