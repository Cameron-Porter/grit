...
/**
 * Checking a set off without typing reps logs the number the empty field was already
 * showing. That placeholder is the HV-045 per-set target when there is history to build one
 * from, so falling back to repsMin instead would silently log fewer reps than the lifter was
 * just shown - and then feed that number back into the next session's targets.
 */
...
/**
 * HV-045: the rep goal for each set of this session, taken from what each set
 * actually did last time. Falls back to the prescribed range when there is no
 * history to build on, so a first exposure still shows something useful.
 */
/**
 * Recent history lists one entry per logged workout, not one per day, so two sessions on the
 * same date - a program day and a Quick Workout, say - render as the same date twice and read
 * as a duplicate. Same-date entries carry their time so they are told apart.
 */
export const historySessionLabels = (sessions:{date:string}[]):string[] => {
  const dates = sessions.map((session) => new Date(session.date)), labels = dates.map((date) => date.toLocaleDateString());
  const repeated = new Set(labels.filter((label,index) => labels.indexOf(label) !== index));
  return labels.map((label,index) => repeated.has(label) ? `${label} · ${dates[index].toLocaleTimeString([],{ hour:'numeric',minute:'2-digit' })}` : label);
};

export function setRepPlan(
  sessions: ExerciseHistorySession[] | undefined,
  setCount: number,
  repsMin: number,
  repsMax: number,
): number[] | null {
  ...
  return perSetRepTargets(last.sets.map((set) => set.reps), setCount, repsMax, repsMin);
}
...
export function WorkoutLogger({ workout, userId, catalog:initialCatalog=[], historyByExercise={} }:{ workout:WorkoutPrescription; userId:string; catalog?:ExerciseOption[]; historyByExercise?:Record<string,ExerciseHistorySession[]> }) {
  ...
  /* One plan per exercise, shared by the rep placeholder and by completeSet, so the number
     shown in an empty field is exactly the number checking it off logs. */
  const repPlans = useMemo(() => exercises.map((exercise,index) => setRepPlan(historyByExercise[exercise.name],draft[index]?.length ?? 0,exercise.repsMin,exercise.repsMax)),[exercises,draft,historyByExercise]);
  ...
  const completeSet = (exerciseIndex:number,setIndex:number,complete:boolean) => {
    ...
    const next=draft.map((sets,index)=>index===exerciseIndex?sets.map((set,position)=>position===setIndex?{...set,complete,reps:complete?completedSetReps(set.reps,repPlans[exerciseIndex]?.[setIndex] ?? exercise.repsMin):set.reps}:set):sets),muscle=exercise?.muscleGroup;
    setDraft(next);
  ...
  return <div className="native-workout-screen">
    ...
    {historySheet!==null&&<div className="modal-backdrop" role="presentation" onClick={()=>setHistorySheet(null)}><section ref={(node)=>{historyDialogRef.current=node}} tabIndex={-1} className="feedback-modal exercise-action-modal" role="dialog" aria-modal="true" aria-labelledby="history-title" onClick={(event)=>event.stopPropagation()} onKeyDown={(event)=>{if(event.key==='Escape')setHistorySheet(null)}}><h2 id="history-title">Recent history</h2><p>{exercises[historySheet]?.name}</p><div className="native-exercise-history">{(()=>{const sessions=(historyByExercise[exercises[historySheet]?.name ?? '']??[]).slice(0,5),labels=historySessionLabels(sessions);return sessions.length?sessions.map((session,sessionIndex)=><div key={sessionIndex}><strong>{labels[sessionIndex]}</strong><ul className="history-set-rows">{session.sets.map((set,index)=><li key={index}><span>Set {index+1}</span><span>{set.weight} lb</span><span>{set.reps} reps</span><span>{set.rir==null?'':`${set.rir} RIR`}</span></li>)}</ul></div>):<p>No recent history for this exercise.</p>})()}</div><div className="modal-actions"><button type="button" className="quiet" onClick={()=>setHistorySheet(null)}>Close</button></div></section></div>}
    {confirmDialog}
  </div>;
}
