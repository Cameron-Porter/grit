-- Defense-in-depth for direct RPC calls: the API route verifies the submitted
-- program day belongs to the caller and exercise names exist in the canonical
-- Supabase exercise catalog before calling save_web_workout(). This replacement
-- keeps the transaction authoritative even if an authenticated client invokes
-- the RPC directly.
...
begin
  ...
  if p_name is null or length(trim(p_name)) = 0 or length(p_name) > 120 then raise exception 'Workout name is required'; end if;
  if p_program_name is null or length(trim(p_program_name)) = 0 or length(p_program_name) > 120 then raise exception 'Program name is required'; end if;
  if jsonb_typeof(p_exercises) <> 'array' or jsonb_array_length(p_exercises) = 0 or jsonb_array_length(p_exercises) > 50 then raise exception 'Invalid exercise payload'; end if;
  if jsonb_typeof(p_feedback) = 'array' and jsonb_array_length(p_feedback) > 40 then raise exception 'Invalid feedback payload'; end if;
  ...
  for v_exercise in select value from jsonb_array_elements(p_exercises) loop
    if coalesce(v_exercise->>'name','') = '' or length(v_exercise->>'name') > 160 then raise exception 'Exercise name is required'; end if;
    if not exists (select 1 from exercises where name = trim(v_exercise->>'name')) then raise exception 'Unknown exercise'; end if;
    if jsonb_typeof(v_exercise->'sets') <> 'array' or jsonb_array_length(v_exercise->'sets') = 0 or jsonb_array_length(v_exercise->'sets') > 20 then raise exception 'Exercise sets must be an array'; end if;
    v_set_index := 0;
    for v_set in select value from jsonb_array_elements(v_exercise->'sets') loop
      if coalesce((v_set->>'completed')::boolean,false) then
        ...
        if v_set->>'rir' is not null and ((v_set->>'rir')::integer < 0 or (v_set->>'rir')::integer > 10) then raise exception 'Invalid prescribed RIR'; end if;
        ...
        values(p_workout_id,trim(v_exercise->>'name'),nullif(v_exercise->>'muscleGroup',''),nullif(v_exercise->>'musclePriority',''),nullif(v_exercise->>'equipment',''),nullif(v_exercise->>'note',''),v_exercise_index,v_set_index,(v_set->>'reps')::integer,(v_set->>'weight')::numeric,true,nullif(v_set->>'rir','')::integer,nullif(v_set->>'reportedRir','')::integer);
      end if;
  ...
  if jsonb_typeof(p_feedback) = 'array' then
    for v_feedback in select value from jsonb_array_elements(p_feedback) loop
      if coalesce(v_feedback->>'muscleGroup','') <> '' then
        if length(v_feedback->>'muscleGroup') > 80 then raise exception 'Invalid feedback muscle'; end if;
